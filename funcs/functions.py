...
def functions(app, message, chat, service, reply_usrname):
    ...
    if mmbr.id != nyanpasu_id:
        ...
        if '!stats' in msgs[0]:        
            ...
            if message.reply_to_message:
                txt = punto(msgs[1], message.reply_to_message.text, service['punt_lang'])
            else:
                txt = punto(msgs[1], message.text, service['punt_lang'])
            txt = reply_usrname + service['punto'] + txt
            app.send_message(chat_id, txt, reply_to_message_id = msg_id)
